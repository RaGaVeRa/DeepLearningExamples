...
#python -m multiproc train.py -m Tacotron2 -o ${OUTPUT_FOLDER} -lr 1e-3 --epochs 6000 -bs 24 --weight-decay 1e-6 --grad-clip-thresh 1.0 --cudnn-enabled --training-files ${TRAIN_FILELIST} --validation-files ${VAL_FILELIST} --log-file ${NVLOGFILE} --anneal-steps 6500 7000 7500 --anneal-factor 0.1 --text-cleaners basic_cleaners --checkpoint-path checkpoints/tacotron2_1032590_6000_amp --drop-text-embedding-weights --epochs-per-checkpoint 10
 python -m multiproc train.py -m Tacotron2 -o ${OUTPUT_FOLDER} -lr 1e-3 --epochs 6501 -bs 24 --weight-decay 1e-6 --grad-clip-thresh 1.0 --cudnn-enabled --training-files ${TRAIN_FILELIST} --validation-files ${VAL_FILELIST} --log-file ${NVLOGFILE} --anneal-steps 7000 7500 8000 --anneal-factor 0.1 --text-cleaners basic_cleaners --epochs-per-checkpoint 10 --resume-from-last
